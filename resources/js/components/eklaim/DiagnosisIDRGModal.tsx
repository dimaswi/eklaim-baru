...
interface DiagnosisIDRGModalProps {
    isOpen: boolean;
    onClose: () => void;
    selectedDiagnosis: DiagnosisIDRG[];
    onSelectDiagnosis: (diagnosis: DiagnosisIDRG) => void;
    onRemoveDiagnosis: (code: string) => void;
}

export default function DiagnosisIDRGModal({
    isOpen,
    onClose,
    selectedDiagnosis,
    onSelectDiagnosis,
    onRemoveDiagnosis
}: DiagnosisIDRGModalProps) {
    const [searchKeyword, setSearchKeyword] = useState('');
    const [searchResults, setSearchResults] = useState<DiagnosisIDRG[]>([]);
    ...
    const handleSearchDiagnosis = async (keyword: string) => {
        ...
        try {
            const response = await fetch(`/eklaim/referensi/diagnosis-idrg?keyword=${encodeURIComponent(keyword)}`);
            const data = await response.json();
            
            if (data.metadata?.code === 200 && data.response?.data) {
                const diagnoses = data.response.data.map((item: any) => ({
            ...
            setSearchResults([]);
            toast.error('Tidak ada data diagnosis IDRG');
        } finally {
            setSearchLoading(false);
        }
    };

    const handleSelectDiagnosis = (diagnosis: DiagnosisIDRG) => {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="!max-w-7xl max-h-[90vh] w-[90vw] top-[35%] overflow-hidden">
                <DialogHeader>
                    <DialogTitle>Pencarian Diagnosis IDRG</DialogTitle>
                </DialogHeader>
                <div className="space-y-4">
                    <Input
                        placeholder="Masukkan keyword untuk mencari diagnosis IDRG (minimal 3 karakter)..."
                        value={searchKeyword}
                        onChange={(e) => {
                    ...
                    {searchLoading && (
                        <div className="flex items-center justify-center py-4">
                            ...
                            <span className="ml-2">Mencari diagnosis IDRG...</span>
                        </div>
                    )}
                                            ...
                                            <tr
                                                ...
                                                className={`${
                                                    !diagnosis.isValid 
                                                        ...
                                                        : selectedDiagnosis.some(d => d.code === diagnosis.code) 
                                                            ? 'bg-blue-50 hover:bg-blue-100' 
                                                            : index % 2 === 0 ? 'bg-white hover:bg-gray-50' : 'bg-gray-25 hover:bg-gray-50'
                                                }`}
                                            >
                                                <td className={`p-2 border-b font-mono text-xs ${!diagnosis.isValid ? 'text-gray-400' : ''}`}>
                                                    {diagnosis.code}
                                                    {!diagnosis.isValid && (
                                                        <span className="ml-1 text-xs text-red-500">(Tidak Valid)</span>
                                                    )}
                                                </td>
                                                <td className={`p-2 border-b ${!diagnosis.isValid ? 'text-gray-400 line-through' : ''}`}>
                                                    {diagnosis.name}
                                                </td>
                                                <td className="p-2 border-b text-center">
                                                    <Button
                                                        size="sm"
                                                        variant="outline"
                                                        onClick={() => diagnosis.isValid && handleSelectDiagnosis(diagnosis)}
                                                        disabled={!diagnosis.isValid}
                                                        className={`text-xs ${
                                                            !diagnosis.isValid 
                                                    ...
                                                    >
                                                        {!diagnosis.isValid 
                                                            ? 'Tidak Valid'
                                                            : selectedDiagnosis.some(d => d.code === diagnosis.code) 
                                                                ? `Pilih Lagi (${selectedDiagnosis.filter(d => d.code === diagnosis.code).length})` 
                                                                : 'Pilih'
                                                        }
                                                    </Button>
                                                </td>
                    ...
                    {searchKeyword.length >= 3 && !searchLoading && searchResults.length === 0 && (
                        <div className="text-center py-8 text-gray-500">
                            ...
                            <p>Tidak ada diagnosis IDRG ditemukan untuk "{searchKeyword}"</p>
                        </div>
                    )}

                    {selectedDiagnosis.length > 0 && (
                        <div className="border-t pt-4">
                            <p className="font-semibold mb-2">Diagnosis IDRG Dipilih ({selectedDiagnosis.length}):</p>
                            <div className="flex flex-wrap gap-2">
                                {/* Group diagnoses by code and show count */}
                                {Object.entries(
                                    selectedDiagnosis.reduce((acc, diagnosis) => {
                                        if (!acc[diagnosis.code]) {
                                            acc[diagnosis.code] = { ...diagnosis, count: 0 };
                                        }
                                        acc[diagnosis.code].count++;
                                        return acc;
                                    }, {} as Record<string, DiagnosisIDRG & { count: number }>)
