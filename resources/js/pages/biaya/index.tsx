...
import { CalendarIcon, Eye, Info, PlusCircle, Search, X } from 'lucide-react';
...
export default function BiayaIndex() {
    const { kunjungan, ruangan_list, filters: initialFilters } = usePage<Props>().props;
            ...
            <div className="max-w-7xl p-4">
                ...
                <div className="mb-4 rounded-lg border bg-gray-50 p-4">
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="date-filter">{dateTypeFilter === 'masuk' ? 'Tanggal Masuk' : 'Tanggal Keluar'}</Label>
                            <Popover>
                            ...
                            {kunjungan.data && kunjungan.data.length > 0 ? (
                                kunjungan.data.map((item: Kunjungan, index: number) => (
                                    <TableRow key={item.id}>
                                        ...
                                        <TableCell>
                                            <Badge variant="outline">{item.noSEP}</Badge>
                                        </TableCell>
                                        ...
                                        <TableCell>
                                            <Button 
                                                ...
                                                size="sm" 
                                                className='mx-2'
                                                onClick={() => router.visit(`/biaya/compare/${item.penjamin?.NOPEN}`)}
                                            >
                                                <Info className="h-4 w-4 text-blue-400" /> Detail
                                            </Button>
                                            {item.klaimStatus !== 1 && (
                                                <PengajuanKlaimModal
                                                    data={{
                                    ...
                                    <TableCell colSpan={7} className="py-8 text-center text-muted-foreground">
                                        <div className="flex flex-col items-center gap-2">
                                            ...
                                            {initialFilters.search && (
                                                <span className="text-sm">Coba ubah kata kunci pencarian atau hapus filter</span>
                                            )}
                                        </div>
                                    </TableCell>
                                </TableRow>
