...
export default function RawatInapResumeMedisRawatJalan() {
    ...
    useEffect(() => {
        if (savedData) {
            ...
            setPasien({
                ...
                lama_rawat: savedData.lama_rawat,
                ...
                riwayat_pengobatan: savedData.riwayat_pengobatan,
                riwayat_penyakit_keluarga: savedData.riwayat_penyakit_keluarga,
                pemeriksaan_fisik: savedData.pemeriksaan_fisik,
                pemeriksaan_penunjang: savedData.pemeriksaan_penunjang,
    ...
    const handleLoadResumeMedis = async () => {
        setLoading('load');
        try {
            const response = await fetch(`/eklaim/rawat-jalan/resume-medis/${kunjungan.NOMOR}`);
    ...
    const handleSimpanResumeMedis = async () => {
        ...
        try {
            const dataToSend = {
                ...
                ...pasien,
                dokter: pasien.dokter || 'Tidak Diketahui',
                petugas: pasien.petugas || '',
                // Format tanggal dengan benar atau null jika kosong atau invalid
                tanggal_lahir: pasien.tanggal_lahir && pasien.tanggal_lahir !== 'Tidak ada' && pasien.tanggal_lahir !== 'Tidak Diketahui' ? pasien.tanggal_lahir : null,
                tanggal_masuk: pasien.tanggal_masuk && pasien.tanggal_masuk !== 'Tidak ada' && pasien.tanggal_masuk !== 'Tidak Diketahui' ? pasien.tanggal_masuk : new Date().toISOString().split('T')[0],
                tanggal_keluar: pasien.tanggal_keluar && pasien.tanggal_keluar !== 'Tidak ada' && pasien.tanggal_keluar !== 'Tidak Diketahui' ? pasien.tanggal_keluar : null,
                jadwal_kontrol_tanggal: pasien.jadwal_kontrol_tanggal && pasien.jadwal_kontrol_tanggal !== 'Tidak Ada' && pasien.jadwal_kontrol_tanggal !== 'Tidak ada' ? pasien.jadwal_kontrol_tanggal : null,
